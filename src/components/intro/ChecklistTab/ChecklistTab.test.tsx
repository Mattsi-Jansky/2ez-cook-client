...
const recipe: Recipe = {
  ...
  ingredients: [
    ...
    { id: "i2", name: "Garlic", quantity: 3, unit: "cloves", note: "Finely minced" },
  ],
  equipment: [
...
describe("ChecklistTab", () => {
  it("renders the intro text", () => {
    render(<ChecklistTab recipe={recipe} checked={{}} onToggle={vi.fn()} portionMultiplier={1} />);
  ...
  it("calls onToggle when an item is clicked", () => {
    ...
    render(<ChecklistTab recipe={recipe} checked={{}} onToggle={onToggle} portionMultiplier={1} />);
  ...
  it("does not render ingredients section when empty", () => {
    ...
    render(<ChecklistTab recipe={noIngredients} checked={{}} onToggle={vi.fn()} portionMultiplier={1} />);
    expect(screen.queryByText("Ingredients")).not.toBeInTheDocument();
  });

  it("does not render equipment section when empty", () => {
    const noEquipment: Recipe = { ...recipe, equipment: [] };
    render(<ChecklistTab recipe={noEquipment} checked={{}} onToggle={vi.fn()} portionMultiplier={1} />);
    expect(screen.queryByText("Equipment")).not.toBeInTheDocument();
  });

  it("passes checked state to items", () => {
    const { container } = render(
      <ChecklistTab recipe={recipe} checked={{ i1: true }} onToggle={vi.fn()} portionMultiplier={1} />,
    );
