...
const recipe: Recipe = {
  ...
  totalTime: "30 min",
  ingredients: [
    { id: "i1", name: "Onion", amount: "1 large" },
    { id: "i2", name: "Garlic", amount: "3 cloves", note: "Finely minced" },
  ],
  equipment: [
...
describe("ChecklistTab", () => {
  it("renders the intro text", () => {
    render(<ChecklistTab recipe={recipe} checked={{}} onToggle={vi.fn()} />);
  ...
  it("calls onToggle when an item is clicked", () => {
    ...
    render(<ChecklistTab recipe={recipe} checked={{}} onToggle={onToggle} />);
  ...
  it("does not render ingredients section when empty", () => {
    ...
    render(<ChecklistTab recipe={noIngredients} checked={{}} onToggle={vi.fn()} />);
    expect(screen.queryByText("Ingredients")).not.toBeInTheDocument();
  });

  it("does not render equipment section when empty", () => {
    const noEquipment: Recipe = { ...recipe, equipment: [] };
    render(<ChecklistTab recipe={noEquipment} checked={{}} onToggle={vi.fn()} />);
    expect(screen.queryByText("Equipment")).not.toBeInTheDocument();
  });

  it("passes checked state to items", () => {
    const { container } = render(
      <ChecklistTab recipe={recipe} checked={{ i1: true }} onToggle={vi.fn()} />,
    );
