...
export function useCookingSession(
  ...
  const [pendingTrackStart, setPendingTrackStart] = useState<string | null>(
    null,
  )
  ...
  const initStage = useCallback(
    (idx: number) => {
      ...
      setStartedTracks(
        new Set(stage.tracks.filter((t) => !t.isParallel).map((t) => t.id)),
      )
      setActiveTrack(
  ...
  const switchToTrack = useCallback((tid: string) => {
    setActiveTrack(tid)
